...
Episode files live at `{cache_dir}/episodes/{feed_id}/{url_id}`. Feed XML is cached at `{cache_dir}/feeds/{feed_id}.rss`. Feed artwork is cached at `{cache_dir}/feeds/{feed_id}-artwork{ext}`.
